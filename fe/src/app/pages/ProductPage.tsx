...
import {
  Star, Truck, Shield, RefreshCw, ChevronRight, ChevronLeft,
...
export function ProductPage() {
  ...
  const submitReview = useMutation({
    ...
    onError: (err) =>
      toast.error(err instanceof ApiError ? err.message : "Không thể gửi đánh giá"),
  });
  ...
  const voteHelpful = useMutation({
    ...
    onError: (err) =>
      toast.error(err instanceof ApiError ? err.message : "Không thể vote"),
  });
  ...
  const submitQuestion = useMutation({
    ...
    onError: (err) =>
      toast.error(err instanceof ApiError ? err.message : "Không thể gửi câu hỏi"),
  });
  ...
  if (!product) return (
    <div className="max-w-7xl mx-auto px-4 py-24 text-center">
      ...
      <button onClick={() => navigate(-1)} className="mt-4 px-6 py-2.5 rounded-xl text-white font-medium" style={{ background: "#00BFB3" }}>
        Quay lại
      </button>
    </div>
  );
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-6">
      ...
      <nav className="flex items-center gap-2 text-sm text-gray-500 mb-6">
        <button onClick={() => navigate("/")} className="hover:text-gray-700">Trang chủ</button>
        <ChevronRight size={14} />
        <button onClick={() => navigate(`/search?cat=${product.category}`)} className="hover:text-gray-700">{product.categoryLabel}</button>
        <ChevronRight size={14} />
      ...
      <div className="grid lg:grid-cols-[500px_1fr] gap-8">
        ...
        <div className="space-y-3">
          <div className="relative bg-white rounded-2xl overflow-hidden shadow-sm" style={{ aspectRatio: "1" }}>
            ...
            {product.badge && (
              <span className="absolute top-3 left-3 px-2.5 py-1 rounded-full text-white text-xs font-bold"
                style={{ background: product.badge === "flash" ? "#E53E3E" : product.badge === "new" ? "#10B981" : "#FF6200" }}>
                {product.badge === "flash" ? "⚡ Flash" : product.badge === "new" ? "Mới" : product.badge === "bestseller" ? "Bán chạy" : "Hot"}
              </span>
            )}
          </div>
        ...
        <div className="space-y-5">
          <div>
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="text-xs text-gray-400 mb-1 uppercase tracking-wider">{product.categoryLabel} · {product.sellerName}</p>
                <h1 className="text-2xl font-bold text-gray-900 leading-snug" style={{ fontFamily: "'Be Vietnam Pro', sans-serif" }}>{product.name}</h1>
              </div>
            ...
            <div className="flex items-center gap-3 mt-3">
              ...
              <button className="text-sm text-gray-500 underline">{product.reviewCount.toLocaleString()} đánh giá</button>
          ...
          {/* Price */}
          <div className="rounded-2xl p-5" style={{ background: "linear-gradient(135deg, rgba(0,191,179,0.06), rgba(255,98,0,0.04))" }}>
            <div className="flex items-end gap-3 mb-2">
              <span className="text-4xl font-black" style={{ color: "#FF6200" }}>{formatPrice(product.price)}</span>
              {product.originalPrice && (
                <span className="text-lg text-gray-400 line-through mb-0.5">{formatPrice(product.originalPrice)}</span>
              )}
              {product.discount && (
                <span className="px-2.5 py-0.5 rounded-full text-sm font-bold text-white mb-1" style={{ background: "#FF6200" }}>
                  -{product.discount}%
                </span>
              )}
            </div>
          ...
          {product.colors && product.colors.length > 0 && (
            <div>
              <p className="text-sm font-semibold text-gray-700 mb-2.5">Màu sắc: <span className="font-normal text-gray-500">{selectedColor}</span></p>
          ...
          {product.sizes && product.sizes.length > 0 && (
            <div>
              <div className="flex items-center justify-between mb-2.5">
                <p className="text-sm font-semibold text-gray-700">Kích cỡ: <span className="font-normal text-gray-500">{selectedSize}</span></p>
                <button className="text-xs font-medium underline" style={{ color: "#00BFB3" }}>Hướng dẫn chọn size</button>
              </div>
              <div className="flex flex-wrap gap-2">
          ...
          {/* Quantity */}
          <div>
            <p className="text-sm font-semibold text-gray-700 mb-2.5">Số lượng: <span className="font-normal text-gray-500">({product.stock} sản phẩm có sẵn)</span></p>
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm text-gray-500">
                Tổng: <span className="font-bold" style={{ color: "#FF6200" }}>{formatPrice(product.price * quantity)}</span>
              </span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              { icon: Truck, text: product.shippingFee === 0 ? "Miễn phí vận chuyển" : `Ship ${formatPrice(product.shippingFee)}`, sub: `Qua ${product.shipping}` },
              { icon: Shield, text: "Bảo vệ quyền lợi", sub: "Hoàn tiền 100%" },
              { icon: RefreshCw, text: "Đổi trả miễn phí", sub: "Trong 30 ngày" },
            ].map((item, i) => (
              <div key={i} className="flex flex-col items-center text-center p-3 rounded-xl bg-gray-50">
        ...
        <div className="mt-8 bg-white rounded-2xl p-5 shadow-sm">
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div className="flex items-center gap-4">
              <ImageWithFallback src={seller.avatar} alt={seller.name} className="w-16 h-16 rounded-2xl object-cover" />
              <div>
                ...
                <div className="flex items-center gap-3 mt-1 text-sm text-gray-500">
                  <span className="flex items-center gap-1"><Star size={13} fill="#F59E0B" className="text-amber-400" /> {seller.rating}</span>
      ...
      <div className="mt-8 bg-white rounded-2xl shadow-sm overflow-hidden">
        <div className="flex border-b border-gray-100">
          {(["desc", "reviews", "qa"] as const).map(tab => (
            ...
            >
              {tab === "desc" ? "Mô tả sản phẩm" : tab === "reviews" ? `Đánh giá (${product.reviewCount.toLocaleString()})` : "Hỏi & Đáp"}
            </button>
          ))}
        </div>
            ...
            <div className="space-y-6">
              ...
              <div className="flex items-center gap-8 p-4 rounded-2xl bg-gray-50">
                <div className="text-center">
                  <p className="text-5xl font-black" style={{ color: "#FF6200" }}>{product.rating}</p>
                  <StarRating value={product.rating} />
                  <p className="text-xs text-gray-500 mt-1">{product.reviewCount.toLocaleString()} đánh giá</p>
                </div>
      ...
      {related.length > 0 && (
        <div className="mt-10">
          <h2 className="text-xl font-bold text-gray-800 mb-5" style={{ fontFamily: "'Be Vietnam Pro', sans-serif" }}>Sản phẩm tương tự</h2>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
            {related.map(p => (
              ...
              >
                ...
                <div className="p-2.5">
                  <p className="text-xs text-gray-600 font-medium line-clamp-2 mb-1">{p.name.split(" ").slice(0, 5).join(" ")}</p>
                  <p className="font-bold text-sm" style={{ color: "#FF6200" }}>{formatPrice(p.price)}</p>
                </div>
              </div>
            ))}
          </div>
