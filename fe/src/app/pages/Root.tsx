...
import {
  ...
  Home, Package, User, LogOut, Settings, Store, LayoutDashboard,
  ChevronDown, Sparkles, Phone, MapPin, Tag, Headphones
} from "lucide-react";
...
function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full" style={{ background: isDark ? "#0f1117" : "linear-gradient(to right, #00BFB3, #009990)" }}>
      {/* Top bar */}
      <div className="border-b border-white/10 hidden md:block">
        <div className="max-w-7xl mx-auto px-4 py-1.5 flex items-center justify-between text-white/80 text-xs">
          <div className="flex items-center gap-6">
            <span className="flex items-center gap-1.5"><Phone size={11} /> 1800 6789 (Miễn phí)</span>
            <span className="flex items-center gap-1.5"><MapPin size={11} /> Giao hàng toàn quốc</span>
          </div>
          <div className="flex items-center gap-5">
            <button onClick={() => navigate("/seller")} className="flex items-center gap-1 hover:text-white transition-colors">
              <Store size={11} /> Kênh Người Bán
            </button>
            <button onClick={() => navigate("/admin")} className="flex items-center gap-1 hover:text-white transition-colors">
      ...
      <div className="max-w-7xl mx-auto px-4 py-3">
        <div className="flex items-center gap-4">
          ...
          >
            <div className="w-9 h-9 rounded-xl flex items-center justify-center shadow-lg" style={{ background: "rgba(255,255,255,0.2)" }}>
              <Sparkles className="w-5 h-5 text-white" />
            </div>
            <div>
              <span className="text-white font-bold text-xl tracking-tight" style={{ fontFamily: "'Be Vietnam Pro', sans-serif" }}>VNShop</span>
              <div className="text-white/60 text-[9px] leading-none tracking-widest">MARKETPLACE</div>
            </div>
          </button>
          ...
          <div className="flex items-center gap-1 ml-auto sm:ml-0 shrink-0">
            ...
            >
              ...
              {cartCount > 0 && (
                <span className="absolute -top-0.5 -right-0.5 min-w-[16px] h-4 px-0.5 rounded-full text-white text-[10px] font-bold flex items-center justify-center" style={{ background: "#FF6200" }}>
                  {cartCount}
                </span>
              )}
            </button>
              ...
              <div className="relative ml-1">
                ...
                >
                  <ImageWithFallback
                    ...
                    placeholder={
                      <div className="w-full h-full rounded-full flex items-center justify-center text-xs font-bold text-white" style={{ background: "rgba(255,255,255,0.2)" }}>
                        {user?.name?.charAt(0)?.toUpperCase() ?? "?"}
                      </div>
                    }
                  />
                  <span className="hidden md:block text-sm font-medium max-w-[80px] truncate">{user?.name?.split(" ").pop()}</span>
                ...
                <AnimatePresence>
                  {userMenuOpen && (
                    <motion.div
                      ...
                      style={{ background: isDark ? "#1a1d2b" : "#fff", borderColor: isDark ? "#2a2d3b" : "#e5e7eb" }}
                    >
                      <div className="p-4 border-b" style={{ borderColor: isDark ? "#2a2d3b" : "#e5e7eb" }}>
                        <p className="font-semibold text-sm" style={{ color: isDark ? "#fff" : "#111" }}>{user?.name}</p>
                        <p className="text-xs mt-0.5" style={{ color: isDark ? "#9ca3af" : "#6b7280" }}>{user?.email}</p>
                      </div>
                      {[
                      ...
                      <div className="border-t" style={{ borderColor: isDark ? "#2a2d3b" : "#e5e7eb" }}>
                        <button
          ...
          >
            <div className="p-4 space-y-1">
              <form onSubmit={handleSearch} className="flex gap-2 mb-3">
                ...
                />
                <button type="submit" className="px-4 py-2 rounded-lg text-white" style={{ background: "#FF6200" }}>
                  <Search size={16} />
                </button>
              </form>
              {[
  ...
  return (
    <div className="min-h-screen" style={{ background: isDark ? "#0f1117" : "#f4f6f9" }}>
      ...
      <footer
        ...
        style={{ background: isDark ? "#0a0c12" : "#1a1d2b", borderColor: isDark ? "#2a2d3b" : "#2a2d3b" }}
      >
        <div className="max-w-7xl mx-auto px-4 py-12">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            <div>
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="text-white font-bold text-lg" style={{ fontFamily: "'Be Vietnam Pro', sans-serif" }}>VNShop</span>
              </div>
              <p className="text-gray-400 text-sm leading-relaxed">Nền tảng mua sắm trực tuyến hàng đầu Việt Nam. Uy tín - Nhanh chóng - Tiết kiệm.</p>
              <div className="flex gap-3 mt-4">
                {["fb", "ig", "tw", "yt"].map(s => (
                  <div key={s} className="w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold text-white cursor-pointer hover:opacity-80 transition-opacity" style={{ background: "#00BFB3" }}>{s.toUpperCase()}</div>
                ))}
              </div>
            </div>
            {[
              {
                ...
                links: ["Trung tâm hỗ trợ", "Chính sách bảo hành", "Hướng dẫn mua hàng", "Phương thức thanh toán", "Vận chuyển & Giao nhận"]
              },
              {
                title: "VNShop",
                links: ["Giới thiệu", "Tuyển dụng", "Điều khoản dịch vụ", "Chính sách bảo mật", "Blog & Tin tức"]
              },
              {
              ...
              <div key={col.title}>
                ...
                <ul className="space-y-2.5">
                  {col.links.map(link => (
                    <li key={link}>
                      <button className="text-gray-400 text-sm hover:text-white transition-colors text-left">{link}</button>
                    </li>
                  ))}
                </ul>
              </div>
          ...
          <div className="mt-10 pt-6 border-t border-gray-700 flex flex-col sm:flex-row items-center justify-between gap-4">
            <p className="text-gray-500 text-xs">© 2026 VNShop. Được vận hành bởi VNShop JSC. ĐKKD số: 0123456789</p>
            <div className="flex items-center gap-3">
              ...
              {["DMCA", "BoCongThuong", "SSL"].map(b => (
                <div key={b} className="px-3 py-1.5 rounded-lg border border-gray-700 text-gray-500 text-xs">{b}</div>
              ))}
            </div>
          </div>
        </div>
